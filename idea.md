...
可以用，但更推荐改成 **normalized weighted MSE**：

```python
loss = (weight * (v_pred - v_target).pow(2)).sum() / weight.sum()
```

原因是这样不同 `alpha` 下 loss 的整体尺度更可比，不会因为把裂隙像素加权了，学习率等效也被悄悄改变。
...
所以如果 DFN 真的是极稀疏线段，`alpha=2,5,10` 是温和实验；**不一定足够强**。先试：
...
如果图像是 binary (0/1)，`threshold=0.0` 没问题。
